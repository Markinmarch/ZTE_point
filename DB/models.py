...
class User(Base):
    ...
    Объект "User" - структура таблицы БД для пользователей.
        Параметры:
            id(int): идентефикатор пользователя: int;
            name(len(str) <= 40): имя пользователя: ;
            phone(str): номер телефона пользователя: str;
            email(str): адрес электронной почты: str;
            password(str): пароль от личного кабинета: str;
